...
class Json_Controller extends Template_Controller
{
	...
	public function cluster()
	{
		...
		if ($category_id != 0)
		{
			...
			{
				$result = $db->query("SELECT count(id) as count FROM monitor_report
								WHERE incident_id = ${row['id']}");	
				$row['is_monitor'] = $result[0]->count;
				$markers[] = $row;
			}
			*/
		...
		foreach ($clusters as $cluster)
		{
			...
			$json_item .= "\"link\": \"".url::base()."reports/index/?c=".$category_id."&sw=".$southwest."&ne=".$northeast."\", ";
		...
		{
			/*
			if ($single['is_monitor'])
			{
				//$color = Kohana::config('settings.monitor_marker_color');	
				$color = '000000';	
			}
			*/
